 ...
 */
public class PayVUtils {
    static PayVUtils payUtils;
    static Context context;

    @SuppressWarnings("static-access")
    public static PayVUtils newInstance(Context context)
    {
        if (payUtils == null)
        {
            payUtils = new PayVUtils();
        }
    ...
    public void alipayPay(String orderId, String orderContent, String orderPrice, final Handler mHandler)
    {
        ...
        Runnable payRunnable = new Runnable()
        {
            ...
            public void run()
            {
                LogUtils.e("payRunnable---start");
                // 构造PayTask 对象
                PayTask alipay = new PayTask((Activity) context);
                // 调用支付接口
                String result = alipay.pay(payInfo);
                LogUtils.e("result---end"+result);
                Message msg = new Message();
                msg.what = SDK_PAY_FLAG;
                msg.obj = result;
                mHandler.sendMessage(msg);
                LogUtils.e("payRunnable---end");
            }
        };
    ...
    public String getOrderInfo(String subject, String body, String price)
    {
        ...
        // 服务器异步通知页面路径
        orderInfo += "&notify_url=" + "\"" + "http://api.shuxin.net/service/JsonData.aspx?action=submit&client_name=vane&method=buy_member_info&moduleid=vane&anonymous=true" + "\"";
